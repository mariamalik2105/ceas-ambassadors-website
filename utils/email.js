...
async function sendSignupNotification(event, member, status) {
  ...
  try {
    ...
    const payload = {
      ...
      callTime: event.call_time ? new Date(event.call_time).toISOString() : '',
      ...
      eventCallTime: event.call_time ? new Date(event.call_time).toLocaleTimeString('en-US', { timeZone: 'America/New_York', hour: '2-digit', minute: '2-digit' }) : '',
